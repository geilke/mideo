...
import org.kramerlab.mideo.classifiers.bayes.NaiveBayes;
import org.kramerlab.mideo.classifiers.core.attributeclassobservers.NominalAttributeClassObserver;
import org.kramerlab.mideo.classifiers.core.attributeclassobservers.GaussianNumericAttributeClassObserver;

/**
 * Hoeffding Tree or VFDT.
 *
 * <p>A Hoeffding tree is an incremental, anytime decision tree
 * induction algorithm that is capable of learning from massive data
 * streams, assuming that the distribution generating examples does not
 * change over time. Hoeffding trees exploit the fact that a small
 * sample can often be enough to choose an optimal splitting
 * attribute. This idea is supported mathematically by the Hoeffding
 * bound, which quantiﬁes the number of observations (in our case,
 * examples) needed to estimate some statistics within a prescribed
 * precision (in our case, the goodness of an attribute).</p>
 *
 * <p>A theoretically appealing feature of Hoeffding Trees not shared by
 * other incremental decision tree learners is that it has sound
 * guarantees of performance. Using the Hoeffding bound one can show
 * that its output is asymptotically nearly identical to that of a
 * non-incremental learner using inﬁnitely many examples. See for
 * details:</p>
 *
 * <p>G. Hulten, L. Spencer, and P. Domingos. Mining time-changing data
 * streams.  In KDD’01, pages 97–106, San Francisco, CA, 2001. ACM
 * Press.</p>
 *
 * <p>Parameters:</p> <ul> <li> -m : Maximum memory consumed by the
 * tree</li> <li> -n : Numeric estimator to use : <ul> <li>Gaussian
 * approximation evaluating 10 splitpoints</li> <li>Gaussian
 * approximation evaluating 100 splitpoints</li> <li>Greenwald-Khanna
 * quantile summary with 10 tuples</li> <li>Greenwald-Khanna quantile
 * summary with 100 tuples</li> <li>Greenwald-Khanna quantile summary
 * with 1000 tuples</li> <li>VFML method with 10 bins</li> <li>VFML
 * method with 100 bins</li> <li>VFML method with 1000 bins</li>
 * <li>Exhaustive binary tree</li> </ul> </li> <li> -e : How many
 * instances between memory consumption checks</li> <li> -g : The number
 * of instances a leaf should observe between split attempts</li> <li>
 * -s : Split criterion to use. Example : InfoGainSplitCriterion</li>
 * <li> -c : The allowable error in split decision, values closer to 0
 * will take longer to decide</li> <li> -t : Threshold below which a
 * split will be forced to break ties</li> <li> -b : Only allow binary
 * splits</li> <li> -z : Stop growing as soon as memory limit is
 * hit</li> <li> -r : Disable poor attributes</li> <li> -p : Disable
 * pre-pruning</li> <li> -l : Leaf prediction to use: MajorityClass
 * (MC), Naive Bayes (NB) or NaiveBayes adaptive (NBAdaptive).</li> <li>
...
public class HoeffdingTree extends AbstractClassifier {
    ...
    public static class Node extends AbstractMOAObject {
        ...
        protected List<Integer> parents;  // EXTENSION
        protected long instanceCount;  // EXTENSION

        public Node(double[] classObservations) {
            observedClassDistribution = new DoubleVector(classObservations);
            parents = new ArrayList<>();  // EXTENSION
            instanceCount = 0;  // EXTENSION
        }
        ...
        /**
         * @param parents attribute indices of this node's parents
         * EXTENSION
         */
        ...
        /**
         * @return attribute indices of this node's parents
         * EXTENSION
         */
        ...
        /**
         * @return number of instances that passed through this node
         * EXTENSION
         */
        ...
        /**
         * @param instanceCount add count to node's instanceCount
         * EXTENSION
         */
        public void addToInstanceCount(long instanceCount) {
    ...
    public static class SplitNode extends Node {
        ...
        protected DoubleVector childrenDistribution;  // EXTENSION
        ...
        public SplitNode(InstanceConditionalTest splitTest,
            ...
            this.childrenDistribution = new DoubleVector();
        }
        ...
        /**
         * Returns a distribution describing how likely it is to select a
         * specific child.
         * @return distribution over children
         * EXTENSION
         */
        public double[] getChildrenDistribution() {
            double dist[] = new double[childrenDistribution.numValues()];
            double sum = childrenDistribution.sumOfValues();
            for (int i = 0; i < dist.length; i++) {
                dist[i] = childrenDistribution.getValue(i) / sum;
            }
            return dist;
        }
        ...
        public FoundNode filterInstanceToLeaf(Instance inst, SplitNode parent,
            ...
            if (childIndex >= 0) {
                childrenDistribution.addToValue(childIndex, 1);  // EXTENSION
    ...
    /**
     * Sets the laplace correction for instances of {@code DoubleVector}
     * to 1.
     * EXTENSION
     */
    public HoeffdingTree() {
        DoubleVector.setLaplaceCorrection(1);
    }
    ...
    /**
     * @return the root of the Hoeffding tree
     * EXTENSION
     */
    public Node getRoot() {
        return treeRoot;
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        if (leafNode == null) {
            ...
            // EXTENSION START
            List<Integer> parents = foundNode.parent.getParents();
            List<Integer> parentsCopy = new ArrayList<>(parents);
            leafNode.setParents(parentsCopy);
            // EXTENSION END
    ...
    public double[] getVotesForInstance(Instance inst) {
        if (this.treeRoot != null) {
            ...
            for (int i = 0; i < dist.length; i++) {
                dist[i] = DoubleVector.getLaplaceCorrection();
            }
            return dist;
          }
    }
    ...
    protected SplitNode newSplitNode(InstanceConditionalTest splitTest,
                                     double[] classObservations, int size, 
                                     long instanceCount) {
        SplitNode node = new SplitNode(splitTest, classObservations, size);
        node.addToInstanceCount(instanceCount);
        return node;
    }
    ...
    protected SplitNode newSplitNode(InstanceConditionalTest splitTest,
                                     double[] classObservations, 
                                     long instanceCount) {
        SplitNode node = new SplitNode(splitTest, classObservations);
        node.addToInstanceCount(instanceCount);
        return node;
    }
    ...
    protected AttributeClassObserver newNumericClassObserver() {
        AttributeClassObserver numericClassObserver = (AttributeClassObserver) new GaussianNumericAttributeClassObserver();
    ...
    protected void attemptToSplit(ActiveLearningNode node, SplitNode parent,
        ...
        if (!node.observedClassDistributionIsPure()) {
            ...
            // EXTENSION START
            List<AttributeSplitSuggestion> originalSuggestions;
            ...
            for (int i = 0; i < bestSplitSuggestions.length; i++) {
                if (Collections.frequency(node.getParents(), i) <= SPLIT_LIMIT) {
                    filtered.add(bestSplitSuggestions[i]);
                }
            }
            AttributeSplitSuggestion[] toArray;
            toArray = new AttributeSplitSuggestion[filtered.size()];
            for (int i = 0; i < filtered.size(); i++) {
                toArray[i] = filtered.get(i);
            }
            bestSplitSuggestions = toArray;
            // EXTENSION END
            ...
            if (shouldSplit) {
                ...
                if (splitDecision.splitTest == null) {
                    ...
                    // EXTENSION START
                    // find attribute corresponding to split suggestion
                    int attIndex = 0;
                    for (int j = 0; j < originalSuggestions.size(); j++) {
                        if (splitDecision == originalSuggestions.get(j)) {
                            attIndex = j;
                            break;
                        }
                    }
                    // add parents to new children
                    List<Integer> parents = node.getParents();
                    List<Integer> parentsCopy = new ArrayList<>(parents);
                    for (int i = 0; i < splitDecision.numSplits(); i++) {
                        double[] dist = splitDecision.resultingClassDistributionFromSplit(i);
                        Node newChild = newLearningNode(dist);
                        newChild.setParents(parentsCopy);
                        newChild.getParents().add(attIndex);
